...
def get_network_config() -> dict:
    raw = (os.getenv("ALGORAND_NETWORK") or "testnet").strip().lower()
    if raw not in NETWORKS:
        raw = "testnet"

    pay_to = (os.getenv("PAY_TO") or os.getenv("ALGORAND_PROVIDER_ADDRESS") or "34N2524PVZG4V6WETGKLTTJ65G6F4LMM64VDXPUBY7YSOCOAFU3KE37XW4").strip()
